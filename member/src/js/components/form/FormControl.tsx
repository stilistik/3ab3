...
export const FormControl: React.FC<FormControlProps> = ({ children, error, fullWidth = true, ...rest }) => {
  ...
  return (
    <Box w={fullWidth ? '100%' : 'auto'} {...rest}>
      <MuiFormControl fullWidth={fullWidth} error={hasError}>
        {children}
        {!disableHelpText && hasError ? <FormHelperText role="alert">{error.message}</FormHelperText> : null}
      </MuiFormControl>
    </Box>
  );
};
